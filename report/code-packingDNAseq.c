...
void packSequence(const unsigned char *seq_to_pack, unsigned char *m_data, int m_len){
    ...
    for ( ; j <= m_len - 4; i++, j+=4 ) 
		m_data[i] = convertFourMerToPackedCode( 
					( unsigned char * ) ( seq_to_pack + j )) ;
    ...
    for(ind = 0; ind < remainder; ind++) 
		blockSeq[ind] = seq_to_pick[j + ind];
    m_data[i] = convertFourMerToPackedCode(blockSeq);}

void unpackSequence(const unsigned char *seq_to_unpack, 
					unsigned char *unpacked_seq, int kmer_len){
	/* Result string is pointer unpacked_seq */
	int i = 0, j = 0;
   	int packed_len = (kmer_len+3)/4;
	for( ; i < packed_len ; i++, j += 4 )
		*( ( unsigned int * )( unpacked_seq + j ) ) = 
					packedCodeToFourMer[ seq_to_unpack[i] ];
