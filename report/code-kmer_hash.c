...
/* Adds a kmer and its extensions in the hash table 
(note that a memory heap should be preallocated. ) */
int add_kmer(hash_table_t *hashtable, memory_heap_t *memory_heap, 
...
(the k-mer was "just added" in the memory heap at position posInHeap - 1) */
